...
# ---------- Simple solver (BT + forward checking) without heuristic ----------
def solve_backtracking_without_h(csp: CSP, var_order: Optional[List[str]]=None) -> Iterable[Assignment]:
    search_steps = 0
    ...
    def backtrack(idx: int):
        ...
        if idx == len(order):
            yield (dict(assignment),search_steps)
            return
        v = order[idx]
 ...
 # ---------- Simple solver (BT + forward checking) with heuristic ----------
def solve_backtracking_with_h(csp: CSP, var_order: Optional[List[str]]=None) -> Iterable[Assignment]:
    search_steps = 0
    domains = {v: list(ds) for v, ds in csp.domains.items()}
    order = var_order or list(domains.keys())
    cons_by_var: Dict[str, List[Constraint]] = {v: [] for v in domains}
    ...
    def backtrack(idx: int):
        ...
        if idx == len(order):
            yield (dict(assignment),search_steps)
            return
        # This is the 3 lines of code that change the order to MRV heuristic
        unnassigned_vars = order[idx:]
        swap_var = min(unnassigned_vars, key=lambda v: len(domains[v]))
        bvi = order.index(swap_var,idx)
        orig = order[idx]
        order[idx],order[bvi] = order[bvi],order[idx]
        # -------------------------------------------
        v = order[idx]
        for val in domains[v]:
            assignment[v] = val
